...
func take_damage(amount: int) -> void:
	...
	if current_health <= 0:
		...
		if level_node != null and level_node.has_method("handle_castle_destroyed"):
			level_node.call("handle_castle_destroyed")
		queue_free()

func _build_health_bar() -> void:
	var bar = TextureProgressBar.new()
	bar.name = "HealthBar"
	bar.position = Vector2(-80, -70)
...
func _spawn_hit_fx() -> void:
	...
	sprite_fx.global_position = global_position + Vector2(0, -12)
	sprite_fx.scale = Vector2(0.9, 0.9)
	...
	tween.tween_property(sprite_fx, "scale", sprite_fx.scale * 1.25, 0.2).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	tween.tween_property(sprite_fx, "modulate:a", 0.0, 0.4).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
